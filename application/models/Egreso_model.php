...
include_once (APPPATH . "models/BaseModel.php"); 
...
class Egreso_model extends CI_Model
{
  ...
  function guarda($data){
   ...
   public function consulta_egreso_acum($data)
    {
  ...
  function muestra_cuenta($id_empresa)
      {
        ...
        //HAmestica: Ordenar cuentas según tabla cuenta_orden
  ...
  //HAmestica: Mostrar sub cuentas según orden clickeado en la view
  //function muestra_subcuenta($id_empresa)
  function muestra_subcuenta($id_empresa,$orden,$direccion,$fecha_ini,$fecha_fin)
  {
    // $this->db->select('s.id_cuenta,s.id_subcuenta,s.nombre_subcuenta,s.monto_gas_proy,s.estado');
    // $this->db->from('subcuenta s');
    // $this->db->where('s.estado', 1);
    // $this->db->where('s.tipo_movimiento', 2);
  ...
  //HAmestica: Consultar totales movimientos por cuenta/semana
  public function consulta_movimientos_cuenta_semana($data){
    $this->db->select('mov.id_cuenta,mov_det.fecha_ingreso,mov_det.fecha_pago');
    $this->db->select_sum('mov_det.Monto');
    $this->db->from('movimiento_detalle mov_det');
    $this->db->join('movimiento mov', 'mov_det.id_movimiento = mov.id_movimiento', 'right');
    $this->db->join('cuenta cue', 'mov.id_cuenta = cue.id_cuenta', 'right');

    $this->db->group_start();
      $this->db->group_start();
        $this->db->where('mov_det.fecha_ingreso >=', $data['fi']);
        $this->db->where('mov_det.fecha_ingreso <=', $data['ft']);
      $this->db->group_end();
      $this->db->or_group_start();
        $this->db->where('mov_det.fecha_pago >=', $data['fi']);
        $this->db->where('mov_det.fecha_pago <=', $data['ft']);
      $this->db->group_end();
    $this->db->group_end();
    
    // $this->db->where('mov.id_tipo_movimiento',$data['id_tipo_movimiento']);
    $this->db->where('mov.id_empresa_guarda',$data['id_empresa_guarda']);
    $this->db->where('mov.estado',1);

    $this->db->group_by('mov.id_cuenta');
    $this->db->group_by('mov_det.fecha_ingreso');
    // $this->db->group_by('mov_det.fecha_pago');
  ...
  public function get_data_mov_det($data){

    $id_movi = $this->db->escape($data['id_movi'] );

    $query = "SELECT 
    md.id_movimiento_detalle,
    format_date(md.fecha_pago) as fecha_pago,
    nombre_tipo_documento,
    numero_tipo_documento,
    format_number(md.monto, 0) as monto,
    nombre_tipo_estado_movimiento
    FROM movimiento m 
    LEFT JOIN movimiento_detalle md ON md.id_movimiento = m.id_movimiento 
    LEFT JOIN tipo_documento td ON td.id_tipo_documento = md.id_tipo_documento 
    LEFT JOIN tipo_estado_movimiento tem ON tem.id_tipo_estado_movimiento = md.id_tipo_estado_movimiento 
    WHERE m.id_movimiento = $id_movi";
    //echo $query;
  ...
  public function consulta_movimiento($data)
    {
      $fi = $this->db->escape($data['fi'] );
      $ft = $this->db->escape($data['ft'] );
      $ie = $this->db->escape($data['ie'] );
      
      $query = "SELECT 
      count(md.id_movimiento_detalle) as cantidad_md, 
      m.id_movimiento, 
      md.id_movimiento_detalle, 
      md.id_tipo_estado_movimiento, 
      SUM(md.monto) as monto, 
      md.fecha_ingreso, 
      md.fecha_pago, 
      color_tipo_documento, 
      es_prioritario, 
      SUM(monto_cuenta_banco) as monto_cuenta_banco, 
      SUM(monto_cuenta_prestamo) as monto_cuenta_prestamo, 
      SUM(monto_nota_credito) as monto_nota_credito, 
      nombre_tipo_documento, 
      md.numero_tipo_documento, 
      nombre_tipo_estado_movimiento, 
      m.id_cuenta as idc, 
      m.id_subcuenta as ids, 
      ...
      LEFT JOIN tipo_estado_movimiento tem ON tem.id_tipo_estado_movimiento = md.id_tipo_estado_movimiento 
      LEFT JOIN tipo_documento td ON td.id_tipo_documento = md.id_tipo_documento 
      WHERE ((md.fecha_ingreso BETWEEN $fi AND $ft) OR (md.fecha_pago BETWEEN $fi AND $ft)) 
      AND m.id_tipo_movimiento = 2 
      AND m.id_empresa_guarda = $ie 
      AND m.estado = 1 
      GROUP BY md.id_movimiento";
/*
      $query = "SELECT 
      -- count(md.id_movimiento_detalle) as cantidad_md, 
      m.id_movimiento, 
      ...
      color_tipo_documento, 
      es_prioritario, 
      -- SUM(monto_cuenta_banco) as 
      monto_cuenta_banco, 
      -- SUM(monto_cuenta_prestamo) as 
      monto_cuenta_prestamo, 
      -- SUM(monto_nota_credito) as 
      monto_nota_credito, 
      nombre_tipo_documento, 
      md.numero_tipo_documento, 
      nombre_tipo_estado_movimiento, 
...
*/
      //echo $query;
      /*$this->db->select('count(md.id_movimiento_detalle) as cantidad_md, m.id_movimiento,md.id_movimiento_detalle,md.id_tipo_estado_movimiento,SUM(md.monto) as monto, md.fecha_ingreso,md.fecha_pago,color_tipo_documento,es_prioritario,SUM(monto_cuenta_banco) as monto_cuenta_banco, SUM(monto_cuenta_prestamo) as monto_cuenta_prestamo, SUM(monto_nota_credito) as monto_nota_credito, nombre_tipo_documento, md.numero_tipo_documento,nombre_tipo_estado_movimiento, m.id_cuenta as idc, m.id_subcuenta as ids, m.id_tipo_movimiento,m.id_empresa_guarda');
      $this->db->from('movimiento m');
     ...
     /* $this->db->where('m.id_tipo_movimiento', 2);
      $this->db->where('m.id_empresa_guarda', $data['id_empresa_guarda']);
      $this->db->where('m.estado',1);
      $this->db->group_by('md.id_movimiento');*/
      /*$this->db->order_by('m.id_cuenta','asc');
      $this->db->order_by('m.id_subcuenta','asc');*/
      //print_r($this->db); exit;
      //echo $this->db->last_query();

      
      //$query1 = $this->db->get($query);
      $result = $this->db->query($query);
      if($result->num_rows() >  0){
          return $result->result();
      }else{
   ...
   public function consulta_egreso_total($data)
    {
      $this->db->select_sum('md.monto', 'monto_total_sem');
      $this->db->select_sum('md.monto_cuenta_banco', 'monto_banco_total_sem');
      $this->db->select_sum('md.monto_nota_credito', 'monto_nota_credito_total_sem');
    ...
    public function consulta_egreso_total_pagado($data)
    {
      $this->db->select_sum('md.monto', 'monto_total_sem');
      $this->db->select_sum('md.monto_cuenta_banco', 'monto_banco_total_sem');
      $this->db->select_sum('md.monto_nota_credito', 'monto_nota_credito_total_sem');
      ...
      $this->db->where('m.id_tipo_movimiento',$data['id_tipo_movimiento']);
      $this->db->where('m.id_empresa_guarda',$data['id_empresa_guarda']);
      $this->db->where('md.id_tipo_estado_movimiento', 1);
      $this->db->where('m.estado',1);
      $query = $this->db->get();
      return $query->row();
    }

    public function consulta_saldo_pendiente($data){
      $this->db->select_sum('md.monto', 'monto_total_sem');
      $this->db->select_sum('md.monto_cuenta_banco', 'monto_banco_total_sem');
      $this->db->select_sum('md.monto_nota_credito', 'monto_nota_credito_total_sem');
      $this->db->from('movimiento m');
  ...
  public function trae_egresos($id_empresa)
      {
    ...
    private function _get_datatables_query()
  {
    ...
    $this->db->where('movimiento.id_tipo_movimiento',2);
    $query = $this->db->get();
    return $query->result();
    }

  public function get_detalle_egreso($id_movimiento)
  {
  ...
  public function get_up_detalle_egreso($id_movimiento)
  {
    $this->db->select('m.id_movimiento, md.id_tipo_documento, nombre_tipo_documento, md.numero_tipo_documento, nombre_cuenta, m.id_subcuenta, nombre_subcuenta, md.monto, md.monto_cuenta_banco, DATE_FORMAT(md.fecha_ingreso, "%d-%m-%Y") as fecha_ingreso, DATE_FORMAT(md.fecha_pago, "%d-%m-%Y") as fecha_pago, m.estado, nombre_tipo_estado_movimiento');
    $this->db->from('movimiento m');
    $this->db->join('movimiento_detalle md', 'm.id_movimiento = md.id_movimiento');
    $this->db->join('cuenta c', 'c.id_cuenta = m.id_cuenta');
    $this->db->join('subcuenta sc', 'sc.id_subcuenta = m.id_subcuenta');
    $this->db->join('tipo_documento td', 'td.id_tipo_documento = md.id_tipo_documento');
    $this->db->join('tipo_estado_movimiento tem', 'tem.id_tipo_estado_movimiento = md.id_tipo_estado_movimiento');
    $this->db->where('m.id_movimiento', $id_movimiento);
    $query = $this->db->get();
    return $query->result();
  ...
  {
    $this->_get_datatables_query();
    $this->db->select('movimiento.id_movimiento, nombre_cuenta, movimiento.id_subcuenta, nombre_subcuenta, 
      (SELECT SUM(monto) FROM movimiento_detalle s WHERE s.id_movimiento = movimiento.id_movimiento) AS monto_total, 
      (SELECT SUM(monto_cuenta_banco) FROM movimiento_detalle s WHERE s.id_movimiento = movimiento.id_movimiento) AS monto_cuenta_banco, 
      (SELECT COUNT(id_movimiento_detalle) FROM movimiento_detalle md WHERE md.id_movimiento = movimiento.id_movimiento) AS cant_doc, 
      (SELECT MAX(fecha_ingreso) FROM movimiento_detalle md WHERE md.id_movimiento = movimiento.id_movimiento) AS fech_ing, movimiento.estado');
  ...
  public function data_movimiento_sinrango($id_movimiento)
      {
        $this->db->select('m.id_movimiento,md.id_movimiento_detalle,
        m.fecha_registro,m.id_tipo_movimiento,m.id_empresa_guarda,m.id_cuenta,nombre_cuenta,cuenta_banco,monto_cuenta_banco,
        m.id_subcuenta,nombre_subcuenta,md.id_tipo_documento,nombre_tipo_documento,con_iva,es_nota_credito,es_obligatorio,md.numero_tipo_documento,
    ...
    public function devuelve_gas_proy_edit($data)
      {
        $this->db->select('s.monto_gas_proy, c.nombre_cuenta, s.nombre_subcuenta');
        $this->db->from('subcuenta s');
        $this->db->join('cuenta c', 'c.id_cuenta = s.id_cuenta');
        $this->db->where('s.id_cuenta =', $data['cuenta']);
        $this->db->where('s.id_subcuenta =', $data['subcuenta']);
        $this->db->where('s.id_empresa_guarda',$data['id_empresa_guarda']);
      ...
      public function guarda_gas_proy($where, $data)
      {
        $this->db->update('subcuenta', $data, $where);
        return $this->db->affected_rows();
      }

      public function consulta_gasto_proy($data)
      {

        $this->db->select('s.monto_gas_proy');
        $this->db->from('subcuenta s');
        $this->db->where('s.id_cuenta =', $data['idc']);
        $this->db->where('s.id_subcuenta =', $data['ids']);
        $this->db->where('s.id_empresa_guarda',$data['id_empresa_guarda']);
        //$this->db->where('m.id_tipo_movimiento',$data['id_tipo_movimiento']);
        /*
        $this->db->select_sum('md.monto', 'monto_real');
        $this->db->from('movimiento m');
        $this->db->join('movimiento_detalle md', 'md.id_movimiento = m.id_movimiento');
        $this->db->where('m.id_cuenta',$data['idc']);
        $this->db->where('m.id_subcuenta',$data['ids']);
      ...
      public function consulta_gasto_real($data)
      {
        //$this->db->select_sum('md.monto', 'monto_real');
        $this->db->select('md.monto, md.fecha_ingreso, md.fecha_pago');
        $this->db->from('movimiento m');
        $this->db->join('movimiento_detalle md', 'md.id_movimiento = m.id_movimiento');
        $this->db->where('m.id_cuenta',$data['idc']);
        $this->db->where('m.id_subcuenta',$data['ids']);
        $this->db->where('m.id_tipo_movimiento',$data['id_tipo_movimiento']);
        $this->db->where('m.id_empresa_guarda',$data['id_empresa_guarda']);
        /*$this->db->where('md.fecha_pago >=','2019-01-01');
        $this->db->where('md.fecha_pago <=','2019-12-31');*/
        
        
        $this->db->group_start();
        $this->db->group_start();
        $this->db->where('md.fecha_ingreso >=', date("Y").'-01-01');
        $this->db->where('md.fecha_ingreso <=', date("Y").'-12-31');
        $this->db->group_end();
        $this->db->or_group_start();
        $this->db->where('md.fecha_pago >=', date("Y").'-01-01');
        $this->db->where('md.fecha_pago <=', date("Y").'-12-31');
        $this->db->group_end();
        $this->db->group_end();
