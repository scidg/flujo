    ...
    <div class="space-4"></div>
        ...
        <?php if ($cuenta_banco == 1) {?>
          ...
          <div class="row"><!-- Inicia row Es Cuenta Banco -->
            ...
            <input type="hidden" value="<?php echo count($data_movimiento_sinrango);?>" name="cant_mov_det" id="cant_mov_det"/>
        ...
        <?php } else {?>
          ...
          <div class="row" style=""><!-- Inicia row Estado de Documento -->
            ...
            <div class="col-lg-2 col-md-2 col-sm-6 col-xs-12">
              ...
              <select <?php echo $solo_lectura;?> name="id_tipo_estado_movimiento[]" id="<?php echo "id_tipo_estado_movimiento_".$j;?>" class="form-control" onchange="limpmia_pendiente(this.value,this.id);">
                  <?php
...
$("#btnAnular").on(ace.click_event, function() {
        bootbox.confirm("¿Está seguro de <B>ANULAR</B> este Movimiento?", function(result) {
            if(result) {
                $.ajax({
                    url : "<?php echo site_url('anular_ingreso')?>",
                    type: "POST",
                    data: {id_movimiento: id_movimiento},
                    success: function(data)
                        {
                            $(location).attr('href','<?php echo base_url() ?>ingreso/' + id_empresa + '/' + hoy);
                        },
                    error: function (jqXHR, textStatus, errorThrown)
                        {
